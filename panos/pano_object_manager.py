...
class PanoramaObjectManager:
    ...
    def __init__(self, hostname: str, username: str = None, password: str = None,
        ...
        if not self.device_group:
            logger.info(f"Not existing device group: {device_group}")
            ...
            if operation == 'create':
                ...
                if "address_groups" in objects_config:
                    for addr_group in objects_config["address_groups"]:
                        ...
                        if name:
                            group_params = {k: v for k, v in addr_group.items() if k != "name"}
                            if "filter_criteria" in group_params:
                                success = self.create_or_update_dynamic_address_group(name, **group_params)
                            else:
                                success = self.create_or_update_static_address_group(name, **group_params)
    ...
    def list_objects(self, objects_config) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for object_type, object_value in objects_config.items():
                if object_type == "address_objects":
                    ...
                    addresses = AddressObject.refreshall(self.device_group)
                ...
                if object_type == "url_categories":
                    objects = [obj.get('name') for obj in object_value]
                    url_cats = CustomUrlCategory.refreshall(self.device_group)
                ...
                if object_type == "address_groups":
                    objects = [obj.get('name') for obj in object_value]
                    groups = AddressGroup.refreshall(self.device_group)
                    all_grp = [grp.name for grp in groups]
                    for group in groups:
                        if group.name in objects:
                            if group.static_value:
                                members = group.static_value
                            elif group.dynamic_value:
                                members = group.dynamic_value
                            results["success"].append({
                                "type": "address_group",
                                "name": group.name,
                                "members": members,
                    ...
                    fail = [f"address_group_{n}" for n in objects if n not in all_grp]
                    results["fail"].extend(fail)
...
def main():
    ...
    if any(OPERATION == op for op in ['create', 'delete']):
        ...
        logger.info("=" * 60)
        logger.info("Creating/deleting objects")
        logger.info("=" * 60)
